...
describe "ActiveRecord American Gladiator" do
  ...
  context "Powerball" do
    it "returns all items containing Powerball" do
      ...
      # Changeable Start
      variable = "Powerball"
      items = Item.where("name like ?", "%#{variable}%")
      # Changeable End
  ...
  context "Breakthrough and Conquer" do
    it "returns all orders with Footballs and Wrestling Rings" do
      ...
      # Changeable Start
      orders = Order.joins(:items).where('items.name = "Football" OR items.name = "Wrestling Ring"')
      # Changeable End
  ...
  context "The Eliminator" do
    it "returns all orders placed 2 weeks ago" do
      ...
      # Changeable Start
      orders = Order.where("created_at < ?", 2.weeks.ago)
      # Changeable End
  ...
  context "Atlasphere" do
    ...
    it "returns most popular items" do
      ...
      # Changeable Start
      most_popular_items = Item.select("items.id, count(items.id) AS total_items").joins(:orders).group("items.id").order("total_items DESC").limit(2)
      # Changeable Stop
